...
model = pickle.load(open("xgb_delinquency_model.pkl", "rb"))
trained_features = pickle.load(open("xgb_model_features.pkl","rb"))
...
credit_score = st.number_input("Credit Score", min_value=300, max_value=850, value=0)
orig_upb = st.number_input("Original UPB (Loan Amount)", min_value=0, value=0)
dti = st.number_input("Debt-to-Income Ratio (%)", min_value=0.0, max_value=100.0, value=0)
ltv = st.number_input("Loan-to-Value Ratio (%)", min_value=0.0, max_value=100.0, value=0)
ocltv = st.number_input("Original Combined LTV (%)", min_value=0.0, max_value=100.0, value=0)
loan_term = st.number_input("Loan Term (months)", min_value=1, value=0)
mip = st.number_input("Mortgage Insurance Premium (MIP)", min_value=0.0, value=0)
